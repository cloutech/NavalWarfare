...
struct Board {							// Die fuenf Eingangsparameter des Baumes.
...
void Zeichne_Ast(int x, int y, float n, float Tiefe, float Laenge)
{
	int x_rel;
	int y_rel;
	if (StopProcess())return;		// Taste zur Unterbrechung wird abgefragt
...
void Restart()
{
	...
	while (									// Erstellt Clickbare Box zum Restart
		!((mouseclick(&x, &y) == 1) &&
			((x > b - 120) && (x < b - 5)) &&		// Erfolgt Mausklick innerhalb dieser Koordinaten
			((y > h - 40) && (y < h - 5))
			)) {
		printf(".");
		if (StopProcess())break;				// dann Reset
	};
