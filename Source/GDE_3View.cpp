...
void CGDE_3View::OnFileOpen(CString ifilename,int ix, int iy) // ifilename: L"Spielfeld.bmp", koordinaten x,y
{
	// TODO: Fügen Sie hier Ihren Befehlsbehandlungscode ein.
	//TCHAR szFilters[] = _T ("bmp files (*.bmp)|*.bmp|All files (*.*)|*.*||");
	//CFileDialog dlg(FALSE, _T ("bmp"), _T ("*.bmp"), OFN_FILEMUSTEXIST | OFN_HIDEREADONLY , szFilters);
	CString filename = ifilename;
	//L"Spielfeld.bmp";		// Gesetzt, damit 
	//if(dlg.DoModal() == IDOK)
	...
	CGDE_3Doc* pDoc = CGDE_3View::GetDocument();

	dib.Load(filename);
	int ww=dib.GetWidth();
	int hh=dib.GetHeight();
	dib.Draw(&(pDoc->buffer),ix,iy,ww,hh,SRCCOPY);
